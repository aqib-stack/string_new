...
export default function StringerPage() {
  ...
  async function flagRacquet() {
    ...
    await saveInspectionRecord(selectedJob.job_id, {
      ...
      flagged_photo_urls: file ? [file.name] : [],
  ...
  return (
    <main className="container shell premium-shell">
      <section className="hero hero-premium stringer-hero">
        <div className="hero-inner premium-hero-inner">
          ...
          <div className="hero-summary-grid stringer-summary-grid">
            <div className="summary-card glass-card"><span className="small">Needs attention</span><strong>{actionCount} active jobs requiring action</strong><span className="summary-meta">Jobs waiting for your action (requested + received)</span></div>
            ...
            <div className="summary-card glass-card"><span className="small">Ready to withdraw</span><strong>{formatMoney(pendingPayout)}</strong><span className="summary-meta">Net amount available</span></div>
          </div>
        </div>
      </section>
      ...
      <section className="panel-grid stringer-top-grid">
        <div className="card col-7 grid strong section-card"><div className="topbar"><div className="section-heading"><span className="kicker">Business insights</span><h2 className="h2">Customer activity</h2></div><BellRing size={18} /></div><div className="stats premium-queue-stats"><div className="stat"><span className="small">Awaiting payment</span><strong>{jobs.filter((job) => job.status === 'FINISHED' && job.payment_requested_at).length}</strong></div><div className="stat"><span className="small">New players this week</span><strong>{insights.newPlayers}</strong></div><div className="stat"><span className="small">Repeat customers</span><strong>{insights.repeatPlayers}</strong></div></div>{alerts[0]?.note ? <div className="small">Latest alert: {alerts[0].note}</div> : null}</div>
        <div className="card col-5 grid strong section-card"><div className="section-heading"><span className="kicker">Exceptions</span><h2 className="h2">Issues & special attention</h2><p className="p">Flagged racquets, payment problems, and paused jobs.</p></div>{jobs.some((job) => job.status === 'AWAITING_PLAYER' || (job.status === 'FINISHED' && job.payment_requested_at)) ? <div className="list">{jobs.filter((job) => job.status === 'AWAITING_PLAYER' || (job.status === 'FINISHED' && job.payment_requested_at)).slice(0, 4).map((job) => <div key={job.job_id} className="small">{job.owner_name || 'Player'} — {job.status === 'AWAITING_PLAYER' ? 'Awaiting player response' : 'Awaiting payment'}</div>)}</div> : <div className="small">No exceptions right now.</div>}</div>
      </section>
