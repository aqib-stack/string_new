...
export default function PlayerRacquetDetailPage({ params }: { params: Promise<{ racquetId: string }> }) {
  ...
  async function requestStringJob() {
    ...
    if (openJob && openJob.status !== 'PICKED_UP') return setMessage(`An active string job already exists: ${formatJobCode(openJob.job_id)}.`);
    ...
    const job = await createJob({ racquet_id: racquet.racquet_id, owner_uid: user.uid, owner_name: user.name, shop_id: shopId, amount_total: 30, status: 'REQUESTED', request_source: 'PLAYER_PORTAL', proof_photo_url: proofFile ? proofFile.name : '' });
    await updateRacquet(racquet.racquet_id, { string_type: useHybrid ? 'Hybrid setup' : stringType, tension: useHybrid ? 'Hybrid setup' : tension, is_hybrid: useHybrid, hybrid_setup: useHybrid ? { mains_string: mainsString, mains_tension: mainsTension, crosses_string: crossesString, crosses_tension: crossesTension } : undefined, racquet_name: racquetName || racquet.racquet_name, racquet_model: racquetModel || racquet.racquet_model, preferred_shop_id: shopId, preferred_shop_name: shopName } as any);
    await addAlert({ type: 'dropoff_request', shop_id: shopId, job_id: job.job_id, racquet_id: racquet.racquet_id, tag_id: racquet.tag_id, owner_name: user.name, note: proofFile ? `Proof photo attached: ${proofFile.name}` : '', created_at: new Date().toISOString(), read: false });
    setMessage('String job requested. Your current stringer has been notified.'); setProofFile(null); await refresh();
  }
  ...
  const primaryAction = useMemo(() => {
    ...
    if (latestJob.status === 'FINISHED') return { type: 'link' as const, label: 'Ready for payment', href: `/player/payment/${latestJob.job_id}` };
  ...
  if (user.user_role !== 'PLAYER') return <main className="container"><div className="card grid"><h1 className="h2">Player portal only</h1><p className="p">This account is configured as a stringer.</p><Link className="btn" href="/auth?mode=signin&role=PLAYER">Open player sign in</Link></div></main>;
