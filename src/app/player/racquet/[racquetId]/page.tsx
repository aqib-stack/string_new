...
export default function PlayerRacquetDetailPage({ params }: { params: Promise<{ racquetId: string }> }) {
  ...
  async function requestStringJob() {
    ...
    const job = await createJob({ racquet_id: racquet.racquet_id, owner_uid: user.uid, owner_name: user.name, shop_id: shopId, amount_total: 30, status: 'REQUESTED', request_source: 'PLAYER_PORTAL', proof_photo_url: proofFile ? proofFile.name : '', racquet_name: racquetName || racquet.racquet_name || '', racquet_model: racquetModel || racquet.racquet_model || '', preferred_shop_name: shopName || shopId, string_type: useHybrid ? 'Hybrid setup' : stringType, tension: useHybrid ? 'Hybrid setup' : tension, is_hybrid: useHybrid, hybrid_setup: useHybrid ? { mains_string: mainsString, mains_tension: mainsTension, crosses_string: crossesString, crosses_tension: crossesTension } : undefined });
